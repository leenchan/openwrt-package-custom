...
var app = 'qbittorrent-bin';
var configuration = {},
	default_options = {},
	status = {};

function callInitScript(action, param) {
	return fs.exec_direct('/etc/init.d/qbittorrent-bin', [action].concat(param ? param : []), 'json');
}
		...
		for (var i = 0; i < options.length; i++) {
			if (Array.isArray(options[i])) {
				var name = options[i][0],
					...
					type = options[i][3],
					props = options[i][4];
				var o = t ? s.taboption(t, type, name, _(title), _(dec)) : s.option(type, name, _(title), _(dec));
				if (name === 'web_ui_password') {
					setValue(o, 'dsdasd');
				}
				if (props) {
					setProps(o, props);
				...
				if (configuration[name] !== undefined) {
					// if (name === 'web_ui_password') {
					// 	setValue(__options[i-1], String(configuration[name]));
						
					// 	console.log(name, o, configuration[name]);
					// } else {
...
return view.extend({
	load: function () {
		return Promise.all([
            callInitScript('get_status'),
            callInitScript('get_conf'),
            callInitScript('api', 'get_preferences'),
            uci.load(app)]
        );
	},
	render: function (data) {
		...
		m = new form.Map('qbittorrent-bin', [_('qBittorrent')], '<br>qBittorrent' + (app_version ? '(' + app_version + ') ' : ' ') + (pid ? _('is running') + (port && ' ' + button_webui) : _('is not running')));
